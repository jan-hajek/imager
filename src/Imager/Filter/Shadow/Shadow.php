...
class Shadow implements Filter
{
	...
	public function apply(Image $image)
	{
		...
		$this->addCorners($canvas, $originalWidth, $originalHeight, $canvasHeight, $canvasWidth);
	...
	protected function addBorders($canvas, $imageWidth, $imageHeight, $canvasHeight, $canvasWidth)
	{
		...
		imagecopyresized($canvas, $r, $canvasWidth - $w, $w + $cornerPadding, 0, 0, $w, $imageHeight - $cornerPadding2, $w, $h);
		
		$b = imagecreatefrompng($this->imageDirectory . "/shadow_B.png");
		imagecopyresized($canvas, $b, $w + $cornerPadding + 1, $canvasHeight - $w, 0, 0, $imageWidth - $cornerPadding2 - 1, $w, $h, $w);
		
		$l = imagecreatefrompng($this->imageDirectory . "/shadow_L.png");
		imagecopyresized($canvas, $l, 0, $w + $cornerPadding, 0, 0, $w, $imageHeight - $cornerPadding2, $w, $h);
	}
	 ...
	 * @param $canvasHeight
	 * @param $canvasWidth
	 * @internal param $imageDirectory
	 * @return int
	 */
	protected function addCorners($canvas, $imageWidth, $imageHeight, $canvasHeight, $canvasWidth)
	{
		$w = 25;
		$h = 25;
